...
const FindAllProfiles = async (req, res) => {
    try {
        const data = await ProfileModel.find().populate('user', ["firstname", "lastname", "email"])
...
const FindSingleProfile = async (req, res) => {
    try {
        const data = await ProfileModel.findOne({user: req.user.id}).populate('user', ["firstname", "lastname", "email"])
        res.status(200).json(data)
    } catch (error) {
        res.status(404).json(error.message)
    }
